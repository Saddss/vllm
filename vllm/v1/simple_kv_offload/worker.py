...
class SimpleCPUOffloadWorker:
    ...
    def register_kv_caches(
        ...
        # Disk (L3) tier: per-rank subdir so TP shards never collide on the
        # same block hash. Bytes are staged through the CPU tensors above.
        if self.disk_offload_path:
            import torch.distributed as dist

            rank = dist.get_rank() if dist.is_initialized() else 0
            root = f"{self.disk_offload_path}/rank{rank}"
            self.disk_tier = DiskTier(
                root, self.cpu_kv_caches, num_io_threads=self.disk_io_threads
            )
