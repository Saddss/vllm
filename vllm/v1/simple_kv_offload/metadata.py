...
class SimpleCPUOffloadMetadata(KVConnectorMetadata):
    ...
    # Disk (L3) staging. Loads pread disk->CPU block; stores pwrite CPU->disk.
    # Keys are per-block filenames derived from the block hash. All disk IO is
    # staged through the worker's CPU tensors and never touches the GPU.
